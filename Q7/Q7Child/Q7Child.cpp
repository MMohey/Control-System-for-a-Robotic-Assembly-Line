...
int	main()
{
	...
	cout << "		#		  Line B			# \n";
	cout << "		~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \n \n";
	CPipe	pipe1("MyPipe1");		// Create a pipe to receive items
	...
	while (1)//start spinning loop
	{
		...
		if (MyDataPool->Red_line == "kill")
		{
			return 0;
		}
		if (MyDataPool->Red_line == "speed")
		{
			no = no + 1;
			MyDataPool->Red_line = "";
		}
		if (MyDataPool->Red_line == "slow")
		{
			no = no - 1;
			MyDataPool->Red_line = "";
		}

		if (pipe1.TestForData() != 0)//test the pipeline befor reading
		{
			if (SchedulerNum == 1)
			{
				pipe1.Read(&d11_FCFS, sizeof(d11_FCFS));	// Read the structure from the pipeline
			...
			if (SchedulerNum == 2)
			{
				...
				pipe1.Read(&d11Priority, sizeof(d11Priority));	// Read the structure from the pipeline

				myVectorPriority.push_back(&d11Priority);
				TEXT_COLOUR(12, 0);						//Change the printing colour to red
				printf("\nProcess %d executed completely, its Wait: %d and TA: %d", d11Priority.process_id, d11Priority.wait, d11Priority.turn_around);
				TEXT_COLOUR(15, 0);						//return the to the default printing colour(whight colour)
