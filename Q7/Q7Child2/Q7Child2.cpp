...
int	main()
{
	...
	cout << "		#		  Line C			# \n";
	cout << "		~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \n \n";
	CPipe	pipe2("MyPipe2");		// Create a pipe to receive items
	...
	while (1)//start spinning loop
	{
		...
		if (MyDataPool->Green_line == "kill")
		{
			return 0;
		}
		if (MyDataPool->Green_line == "speed")
		{
			no = no + 1;
			MyDataPool->Green_line = "";
		}
		if (MyDataPool->Green_line == "slow")
		{
			no = no - 1;
			MyDataPool->Green_line = "";
		}

		if (pipe2.TestForData() != 0)//test the pipeline befor reading
		{
			...
			{
				pipe2.Read(&d11_FCFS, sizeof(d11_FCFS));	// Read the structure from the pipeline
