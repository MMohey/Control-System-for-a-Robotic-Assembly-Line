...
private:
	//##ModelId=3DE6123903CF
	PROCESS_INFORMATION pInfo ;

	//##ModelId=3DE6123903E3
	HANDLE	ProcessHandle ;		// handle to the child Process, can be used to identify a process
	//##ModelId=3DE6123A0005
	HANDLE  ThreadHandle;		// handle to the child process's main thread, can be used to identify a thread

	//##ModelId=3DE6123A000F
	UINT ProcessID ;			// Id of the Child Process (not the same as a handle)
	//##ModelId=3DE6123A0019
	UINT ThreadID;				// Id of the Child Process's main thread (not the same as a handle)

	//##ModelId=3DE6123A002E
...
public:
	...
	//##ModelId=3DE6123A0055
	virtual ~CProcess() {}				// destructor (does nothing by default)
	//##ModelId=3DE6123A005F
	inline HANDLE	GetProcessHandle() const { return ProcessHandle ; }		// get the process's handle
	//##ModelId=3DE6123A0061
	inline HANDLE	GetThreadHandle() const { return ThreadHandle ; }		// get the process's main thread handle

	inline operator string	() const {return ProcessName ;}
	inline string	GetName() const { return ProcessName ; }

	
	//##ModelId=3DE6123A006A
	inline UINT	GetProcessId() const { return ProcessID ; }		// get the process's ID
	//##ModelId=3DE6123A0073
	inline UINT	GetThreadId() const { return ThreadID; }		// get the main thread's ID

	//##ModelId=3DE6123A007D
	...
	BOOL Signal(UINT Message) const ;		// allows a signal/message to be sent to the process

private:
	//##ModelId=3DE6123A00BA
	void Exit(UINT ExitCode) const ;		// called by process to terminate itself

	inline BOOL TerminateProcess(UINT ExitStatus = 0) { return ::TerminateProcess(ProcessHandle, ExitStatus) ; }
} ;
...
int main()
{
	...
	CProcess p1("c:\\users\\paul\\parent\\debug\\paul1",		// pathlist to '.exe.' file we wish to set running
...
public:
	...
	BOOL Signal(UINT Message) const ;		// caller sends a signal/message to the thread
...
int main()
{
	...
	if(c1.Resume() == FALSE)	{	
 	...
 	if(c1.Signal(STOP)  == FALSE)	{
		  perror("Cannot Signal thread") ;
		  exit(0) ;
	}
...
//	Abstract class, should not make one of these, only an instance of a derived one
//	This is included only for compatibility with older programs
//	New program should use one of the derived classes such as 'CEventSingleThreadRelease' and
//	'CEventMultipleThreadRelease'

/*
//##ModelId=3DE6123B009D
class CEvent {				// see EVENT related functions in rt.cpp for details
protected:

	//##ModelId=3DE6123B00A7
	HANDLE	EventHandle ;	// handle to the event
	//##ModelId=3DE6123B00B2
	const string EventName ;
	
public:	
	//##ModelId=3DE6123B00BB
	UINT Wait(DWORD Time = INFINITE) const ;			// perform a wait on an event for ever or until specified time
	//##ModelId=3DE6123B00C5
	BOOL Signal() const ;		// perform a signal on an event
	//##ModelId=3DE6123B00C7
	virtual BOOL Reset() const ;		// reset the event
	//##ModelId=3DE6123B00D0
	BOOL Test() const ;			// see if event has happened
	//##ModelId=3DE6123B00D9
	BOOL Pulse() const ;		// pulse event
	//##ModelId=3DE6123B00E3
	BOOL Unlink() const ;		// unlink from event, i.e. we have finished using it

	//##ModelId=3DE6123B00E5
	HANDLE	GetHandle() const { return EventHandle ; }		// get handle to event

	//##ModelId=3DE6123B00EE
	...
	//##ModelId=3DE6123B00F0
	CEvent(	const string &Name,								// construct an event object by name
			BOOL bType,										// btype can MANUAL or AUTORESET, 
			BOOL bState = NOTSIGNALLED) ;					// bState = SIGNALLED means event is created in the signalled state, 
															// bState = NOTSIGNALLED means not signalled

	//##ModelId=3DE6123B0101
	virtual ~CEvent() { Unlink() ; }					// unlink event, destroyed automatically
};
*/
...
public:	
	...
	CEvent(const string &Name, BOOL bType, BOOL bState = NOTSIGNALLED) ;	// btype = SINGLE_RELEASE or MULTIPLE_RELEASE to allow one or many thread to resume when event is signalled
...
CEvent e1("Paul", MULTIPLE_RELEASE) ;		// a global event which when signalled will release all waiting threads
...
class CMessageQueue								// see Message related functions in rt.cpp for more details
{
public:
...
{	 	
	...
	do {
		...
		if(mq1.TestForMessage() == TRUE) {		// see if a message exists in the message queue
...
{
	UINT Message ;
	CMessageQueue 	mainq ;		// create a message queue for the main process thread
	...
	do {
		...
		if(mainq.TestForMessage() == TRUE) {		// is there a message in main message queue
			...
			if(Message == 2000)				// if message intended for thread2
				Child2.Signal(Message) ;			// send the message to it
			if(Message == 3000)	{			// if message from another process 
				...
				Child2.Signal(2001) ;			// kill child by sending agreed END message
				break ;					// end loop
			}
		}
...
public:
	...
	BOOL KillTimer() ;					// stops a timer from posting messages to threads message queue
...
int main()
{
	...
	do {
		if(mainq.TestForMessage() == TRUE) {				// see if we have got any messages from other processes
			Message = mainq.GetMessage() ;					// if so, read the message
		...
		if(t1.TestForTimer() == TRUE)	{				// if this main threads timer has gone off
			...
			Child1.Signal(END) ;						// tell child thread to stop by sending END message
			Child2.Signal(END) ;						// tell child thread to stop by sending END message
			break ;										// break from loop
		}
		// etc for any other messages
	}while(1) ;											// do forever
		
	t1.KillTimer() ;											// kill off the main threads timer
...
public:
	...
	CRendezvous(const string &TheRendezvousName, int NumberThreads) : RendezvousName(TheRendezvousName)
	{
		RendezvousMutex = new CMutex(string("__RendezvousMutex__") + RendezvousName) ;
		RendezvousEvent = new CEvent(string("__RendezvousEvent__") + RendezvousName, MULTIPLE_RELEASE) ;
...
public:
	...
	CReadersWritersMutex(const string &MyName) : Name(MyName)
	{
...
public:
	...
	CWritersReadersMutex(const string &MyName) : Name(MyName)
	{
		...
		WritersReadersCondition = new CCondition(string("__CWritersReadersCondition__") + MyName) ;
	...
	void WaitToRead()
	{
		...
		if(++(ptr->NumberOfReaders) == 1)		// if this is the 1st reader
			WritersReadersSemaphore->Wait();	// wait on the semaphore as a writer may be using the resource
	...
	void DoneReading() 
	{
		...
		// decrement the number of waiting readers
		if(--(ptr->NumberOfReaders) == 0)			// if no readers
			WritersReadersSemaphore->Signal() ;		// signal that writers may now enter
	...
	void WaitToWrite()
	{
		WritersReadersCondition->Reset() ;
		WritersReadersSemaphore->Wait() ;		// used to exclude all readers
	}
	...
	{		
		PERR((ptr->NumberOfReaders < 2) , "HELP");
		WritersReadersSemaphore->Signal() ;		// allow readers in
		WritersReadersCondition->Signal() ;
	}
...
public:
	...
	CDinningPhilosophers(const string _Name) : Name(_Name), NUMBER_OF_PHILOSOPHERS(5) 
	{
		...
		for(int i = 0; i < NUMBER_OF_PHILOSOPHERS; i ++)
		{
			...
			Event[i] = new CEvent(string("__DinningPhilosophersSemaphore__") + _Name + string(buff), MULTIPLE_RELEASE) ;
		}
...
public:
	...
	CTypedPipe(const string &Name, UINT SizeOfPipe = 1024);			// default constructor
...
//##ModelId=3DE6123D0104
template <class T>
CTypedPipe<T>::CTypedPipe(const string &Name, UINT SizeOfData) 
	:CPipe(Name, SizeOfData)
{}
