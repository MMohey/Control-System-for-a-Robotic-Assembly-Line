		...
		: ProcessName(Name)
{
	...
	ThreadHandle = pInfo.hThread;		// handle to the child process's main thread, can be used to identify a thread
...
BOOL CProcess::SetPriority(int Priority) const
{
	...
	Success = SetPriorityClass(ProcessHandle, Priority) ;
...
{
	UINT Result = WaitForSingleObject(ProcessHandle, Time) ;	
...
BOOL CProcess::Signal(UINT Message) const	// message value and ID of thread
{
	BOOL Result ;
...
BOOL CThread::Signal(UINT Message) const	// message value and ID of thread
{
	BOOL Result ;
	...
	:PipeName(Name)
{
	...
	const string ConSemaName = "____PipelineConsumerSemaphore__" + Name;
	...
	pProdSemaphore = new CSemaphore(ProdSemaName,  0, SizeOfPipe) ;		// create semaphore
	pConSemaphore = new CSemaphore(ConSemaName, SizeOfPipe - 1 , SizeOfPipe - 1) ;		// create semaphore
...
{
	pMutex->Wait() ;
	int NumBytesInPipe = PipePointer->NumBytes ;	// get number of bytes on pipeline

	if(NumBytesInPipe == 0)	{
		PipePointer->Initialised = 0 ;
...
BOOL CPipe::Write(void *Data, UINT Size)	// producer process
{
	...
	for(UINT i = 0; i < Size; i ++)	{
		...
		if(PipePointer->WritingIndex == PipePointer->SizeOfPipe - 1)		// if we have GONE PAST the end of the pipeline buffer
...
BOOL CPipe::Read(void *Data, UINT Size) 
{
	...
	for(UINT i = 0; i < Size; i ++)	{
		...
		if(PipePointer->ReadingIndex == PipePointer->SizeOfPipe - 1)			// if we have GONE PAST the end of the pipeline buffer
...
void CMessageQueue::WaitForMessage() const
{
...
BOOL CMessageQueue::TestForMessage(UINT MinMessageValue, UINT MaxMessageValue) const
{
	MSG	MessageBuff ;
...
UINT	CMessageQueue::GetMessage(UINT MinMessageValue, UINT MaxMessageValue) const
{
	MSG	Message ;
	LPMSG	Mess = &Message ;
...
//	until killed with the Kill() function
//
...
BOOL CTimer::KillTimer()		// kills a timer so that it no longer posts timer messages to thread
{ 
...
{		
	...
	CREATE_MESSAGE_QUEUE() ;			// Create a message queue for this child thread
	
	TimerID = SET_TIMER(500) ;			// set up repetitive timer to go off at 1/2 second intervals

	do {
		if(TEST_FOR_MESSAGE() == TRUE) {	// see if any messages arrived for this thread
			Message = GET_MESSAGE() ;		// if so get the message
		...
		if(TEST_FOR_TIMER() == TRUE)		// if the threads timer has gone off, print message
			printf("ThreadFn1 Got Timer.....\n") ;

	}while(Message != END) ;		// terminate loop and thus thread when END message received
	
	KILL_TIMER(TimerID) ;			// stop the child threads timer
...
int main()
{
	HANDLE	hThread ;				// Create handle for the child thread
	UINT	ThreadID ;				// to hold the threads ID number

	UINT	Message ;
	UINT	TimerID = SET_TIMER(3000);		// repetitive 3 second timer

	CREATE_MESSAGE_QUEUE() ;		// create a message queue for the main process thread

	// Create a child thread

	printf("Parent Thread: Creating Child Thread 1 in Active State\n") ;

	if((hThread = CREATE_THREAD(ThreadFn1,				// name of thread function
								ACTIVE,					// thread is created in ACTIVE state
								&ThreadID,				// address (&) of variable to hold thread ID
	...
	do {
		if(TEST_FOR_MESSAGE() == TRUE) {				// see if we have got any messages from other processes
			Message = GET_MESSAGE() ;					// if so, read the message
			if(Message == JOB1 || Message == JOB2)	{	// if the message if for the child thread
				SIGNAL_THREAD(ThreadID, Message) ;		// send the message to it
			}
			// etc for any other messages
		}
		if(TEST_FOR_TIMER() == TRUE)	{				// if this main threads timer has gone off
			printf("ThreadFn1 Got Timer.....\n") ;
			SIGNAL_THREAD(ThreadID, END) ;				// tell child thread to stop by sending END message
			break ;										// break from loop
		}
		// etc for any other messages
	}while(Message != WM_TIMER) ;		// terminate loop and thus thread when END message received
		
	KILL_TIMER(TimerID) ;				// kill off the main threads timer
	
	// wait for the child thread to die

	printf("Parent Thread: Waiting for Thread 1\n") ;

	if(WAIT_FOR_THREAD(hThread)  == WAIT_FAILED)	{	// if thread already dead, then proceed, otherwise wait for it to finish
